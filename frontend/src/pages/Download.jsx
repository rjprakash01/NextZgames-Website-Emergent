...
export default function DownloadPage() {
  ...
  const Card = ({ id, icon: Icon, title, note, primary }) => (
    ...
    >
      {primary && (
        <span className="absolute right-4 top-4 rounded-full bg-[#284525] px-3 py-1 font-heading text-[10px] font-extrabold uppercase tracking-widest text-[#EFE35F]">
          Your device
        </span>
      )}
      ...
      <h2 className="mt-5 font-heading text-lg font-extrabold text-[#284525]">{title}</h2>
      <p className="mt-1.5 text-sm text-[#284525]/60">{note}</p>
      <button
          ...
          <div className="grid gap-5 md:grid-cols-3">
            ...
            <Reveal delay={0.2}>
              <div data-testid="download-card-qr" className={`card-light relative h-full p-5 ${device === "desktop" ? "!border-[#284525]" : ""}`}>
                ...
                <h2 className="mt-5 font-heading text-lg font-extrabold text-[#284525]">Scan to Download</h2>
                <p className="mt-1.5 text-sm text-[#284525]/60">Point your phone camera at the code to open this page on your device.</p>
                <div className="mt-6 flex justify-center">
                  <div className="rounded-2xl border border-[#284525]/15 bg-white p-4 shadow-[0_10px_30px_rgba(24,43,23,0.08)]" data-testid="download-qr">
                    <QRCodeSVG value={downloadUrl} size={140} bgColor="#ffffff" fgColor="#284525" level="M" />
                  </div>
                </div>
              </div>
            </Reveal>
