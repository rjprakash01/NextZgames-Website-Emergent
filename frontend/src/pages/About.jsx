...
export default function About() {
  ...
  return (
    <>
            ...
            {PILLARS.map((p, i) => (
              <Reveal key={p.title} delay={0.06 * i}>
                <div className="card-light h-full p-5" data-testid={`about-pillar-${i}`}>
                  ...
                  <h3 className="mt-3 font-heading text-base font-bold text-[#1B3A17]">{p.title}</h3>
                  <p className="mt-1.5 text-sm text-[#1B3A17]/60">{p.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="bg-[#1B3A17] py-8 md:py-12" data-testid="about-vision">
