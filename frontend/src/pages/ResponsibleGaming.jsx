            ...
            {ITEMS.map((item, i) => (
              <Reveal key={item.title} delay={0.04 * i}>
                <div className="card-light h-full p-5" data-testid={`rg-card-${i}`}>
                  ...
                  <h3 className="mt-3 font-heading text-base font-bold text-[#1B3A17]">{item.title}</h3>
                  <p className="mt-1.5 text-sm text-[#1B3A17]/60">{item.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
          <Reveal delay={0.2}>
            <div className="mt-7 rounded-2xl border border-[#1B3A17] bg-[#1B3A17] p-5 md:p-8" data-testid="rg-notice">
