...
export default function HowItWorks() {
  ...
  return (
    <>
      ...
      />
      <section className="bg-[#1f361d] py-24 md:py-32" data-testid="steps-section">
        <div className="mx-auto max-w-4xl px-6">
          <div className="relative space-y-14 before:absolute before:bottom-6 before:left-[27px] before:top-6 before:w-px before:bg-[#D4C942]/25 md:before:left-[35px]">
            {HOW_IT_WORKS_STEPS.map((s, i) => (
              <Reveal key={s.n} delay={0.08 * i}>
                <div className="relative flex gap-8 md:gap-12" data-testid={`how-page-step-${s.n}`}>
                  <span className="relative z-10 flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-[#D4C942]/40 bg-[#284525] font-heading text-lg font-extrabold text-[#D4C942] md:h-[72px] md:w-[72px] md:text-2xl">
                    {s.n}
                  </span>
                  <div className="pt-2">
                    <h2 className="font-heading text-2xl font-extrabold text-white">{s.title}</h2>
                    <p className="mt-2 max-w-lg text-sm md:text-base text-white/65">{s.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
