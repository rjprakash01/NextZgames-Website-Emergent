...
const screens = {
  home: (
    <>
      ...
      <div className="mx-3 mt-2 grid grid-cols-2 gap-2">
        <div className="rounded-xl bg-[#1B3A17] border border-[#EFE35F]/30 p-3">
          <p className="font-heading text-[10px] font-bold text-white">POKER</p>
          <p className="text-[7px] text-white/50 mt-1">12 tables live</p>
        </div>
        <div className="rounded-xl bg-[#1B3A17] border border-white/10 p-3">
      ...
      <div className="mx-3 mt-2 rounded-xl bg-gradient-to-r from-[#EFE35F] to-[#CDBF4D] p-3">
        <p className="font-heading text-[9px] font-extrabold text-[#1B3A17]">WELCOME BOOST</p>
        <p className="text-[7px] text-[#1B3A17]/70">New player offer inside</p>
      </div>
      ...
      <div className="mx-3 mt-2 space-y-2">
        {[
          ...
          <div key={name} className="flex items-center justify-between rounded-xl bg-[#30512D]/70 border border-white/8 px-3 py-2.5">
            ...
            <span className="rounded-full bg-[#EFE35F] px-2.5 py-1 font-heading text-[7px] font-extrabold text-[#1B3A17]">JOIN</span>
          </div>
        ))}
      </div>
      <div className="mt-auto mx-3 mb-3 flex justify-between rounded-xl bg-black/30 px-4 py-2 text-[7px] text-white/50">
    ...
    <>
      ...
      <div className="relative mx-3 mt-2 flex-1 rounded-2xl border border-white/15 bg-[radial-gradient(circle_at_center,#2A4C25_0%,#1B3A17_60%,#142C11_100%)]">
        ...
        <div className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 gap-1.5">
          {["A♠", "K♥", "Q♣", "7♦", "2♠"].map((c) => (
            <span key={c} className="flex h-8 w-6 items-center justify-center rounded bg-white font-heading text-[8px] font-extrabold text-[#1B3A17] shadow">{c}</span>
          ))}
        </div>
        <div className="absolute bottom-12 left-1/2 flex -translate-x-1/2 gap-1.5">
          {["9♠", "9♥"].map((c) => (
            <span key={c} className="flex h-9 w-7 items-center justify-center rounded border-2 border-[#EFE35F] bg-white font-heading text-[9px] font-extrabold text-[#1B3A17] shadow-lg">{c}</span>
          ))}
        </div>
        <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5 text-[7px] font-heading font-bold">
          ...
          <span className="rounded-full bg-[#EFE35F] px-3 py-1.5 text-[#1B3A17]">RAISE</span>
        </div>
      </div>
      <div className="mb-3" />
    </>
    ...
    <>
      ...
      <div className="mx-3 mt-2 rounded-xl bg-[#30512D]/70 border border-white/15 p-3">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <span className="rounded-lg bg-[#EFE35F] py-2 text-center font-heading text-[8px] font-extrabold text-[#1B3A17]">MUMBAI · 1.85</span>
      ...
      <div className="mt-auto mx-3 mb-3 rounded-xl bg-[#EFE35F] py-2.5 text-center font-heading text-[8px] font-extrabold tracking-widest text-[#1B3A17]">
  ...
  wallet: (
    <>
      <AppHeader />
      <div className="mx-3 mt-3 rounded-2xl border border-[#EFE35F]/30 bg-gradient-to-br from-[#30512D] to-[#1B3A17] p-4">
        ...
        <div className="mt-3 flex gap-2">
          <span className="rounded-full bg-[#EFE35F] px-3 py-1.5 font-heading text-[7px] font-extrabold text-[#1B3A17]">ADD FUNDS</span>
...
export const PhoneMockup = ({ screen = "home", className = "", testid }) => (
  ...
  >
    <div className="relative aspect-[9/19] w-full overflow-hidden rounded-[2.4rem] bg-[#1B3A17] flex flex-col">
      {screens[screen]}
    </div>
  </div>
);
